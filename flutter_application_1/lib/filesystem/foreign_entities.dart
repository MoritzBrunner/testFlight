...
/// Finds and moves all FileSystemEntities that are different
/// form Bundles, Pages or Page-Content to the "other" directory.
void moveAllAtypicalFileSystemEntitiesToOtherDirectory(Directory root) {
  // in pagesDirectory
...
List<FileSystemEntity> findAtypicalBundles(Directory dir) {
  ...
  reportAtypicalEntities(atypicalEntities, 'bundle', dir);
...
List<FileSystemEntity> findAtypicalPages(Directory dir) {
  ...
  reportAtypicalEntities(atypicalEntities, 'page', dir);
  return atypicalEntities;
}

/// Searches the directory for FileSystemEntities that
/// are not Entries or Images and returns alist of them.
List<FileSystemEntity> findAtypicalPageContent(Directory dir) {
  ...
  reportAtypicalEntities(atypicalEntities, 'pageContent', dir);
  return atypicalEntities;
}
...
void moveFileSystemEntities(
  ...
  if (entities.isNotEmpty) {
    print('moved ${entities.length} entity(s) to $targetDir');
  }
}

/// Helps with debugging. To prevent bloating of other functions.
void reportAtypicalEntities(List atypicalEntities, String type, Directory dir) {
  if (atypicalEntities.isNotEmpty) {
    print('atypical $type(s) found in $dir');
    for (var entitiy in atypicalEntities) {
      print(' - ' + entitiy.toString());
    }
  }
}
...
bool isBundleNameFormat(String name) {
  try {
    ...
    return correctLenght && isYear && isMonth && isDay;
  } catch (_) {
    return false;
  }
}
